        ...
        for x in range(Constants.UNITS):
            ...
            for y in range(Constants.UNITS):
                pavement = PavementBlock(
                    self.unit, PavementBlockStatus.GRASS, (x, y))
        ...
        for x in range(position[0]-radius, position[0]+radius+1):
            for y in range(position[1]-radius, position[1]+radius+1):
                if x >= 0 and x < Constants.UNITS and y >= 0 and y < Constants.UNITS:
                    distance = int(util.points_distance(
...
class PavementBlock(pygame.sprite.DirtySprite):
    ...
    def update_status(self, status):
        if status == PavementBlockStatus.BLOOD and self.status == PavementBlockStatus.BLOOD:
            return
        self.status = status
        self.dirty = 1
        self.refill = 0
        if self.status == PavementBlockStatus.NONE:
            ...
            self.refill = Constants.FPS * \
                (2 + random.randint(-1, 1))  # between 1 and 3 seconds
            ...
            rect_a = pygame.Rect(int((random.randint(0, 10)/10.0)*self.unit),
                                 int((random.randint(0, 10)/10.0)*self.unit), size_a, size_a)
            size_b = int(size*(random.randint(0, 20)/10.0))
            rect_b = pygame.Rect(int((random.randint(0, 10)/10.0)*self.unit),
                                 int((random.randint(0, 10)/10.0)*self.unit), size_b, size_b)
            size_c = int(size*(random.randint(0, 20)/10.0))
            rect_c = pygame.Rect(int((random.randint(0, 10)/10.0)*self.unit),
                                 int((random.randint(0, 10)/10.0)*self.unit), size_c, size_c)
            pygame.draw.rect(
